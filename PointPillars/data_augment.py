...
class DataSet():
    # 这是数据集中所有的类别，但是在我只知道car，pedestrian，cyclist的anchor size，
    # anchor size在整个训练当中很重要，因此，其他类别我识别不了
    # 另外，过多的类别对数据增强和数据集的大小都有着很高的要求
    # 所以我只能简化类型了
    # CLASSES = {
    #     'pedestrian': 0, 
    #     'bicycle': 1, 
    ...
    def generate_data(self, data_info):
        ...
        # 根据标签隐射到对应的label
        ground_truth_labels = [self.CLASSES.get(name, -1) for name in annos_name]       # 直接按annos_name来对应
        
        # # 将所有车都归类为car，将单车摩托车都当成bicycle
        # ground_truth_labels = []
        # for name in annos_name:
        #     if name == 'motorcycle':
        #         name = 'bicycle'
        #     elif name == 'truck' or name == 'bus' or name == 'other_vehicle' or name == 'emergency_vehicle':
        #         name = 'car'
        #     if name == 'animal':
        #         name = 'pedestrian'
        #     ground_truth_labels.append(self.CLASSES.get(name, -1))
    ...
    def data_augment_main(self,data_dict):
        ...
        #                  sample_groups=dict(pedestrian=10, bicycle=10, car=15, truck=10, bus=10, 
        #                                     other_vehicle=10, emergency_vehicle=5,motorcycle=5,
        #                                     animal=5))
